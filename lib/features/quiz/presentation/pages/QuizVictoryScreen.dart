...
class QuizVictoryScreen extends StatefulWidget {
  const QuizVictoryScreen({Key? key}) : super(key: key);
  @override
  State<QuizVictoryScreen> createState() => _QuizVictoryScreenState();
}

class _QuizVictoryScreenState extends State<QuizVictoryScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('34_QuizVictory_ResultsScreen', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
