...
class QuizDefeatScreen extends StatefulWidget {
  const QuizDefeatScreen({Key? key}) : super(key: key);
  @override
  State<QuizDefeatScreen> createState() => _QuizDefeatScreenState();
}

class _QuizDefeatScreenState extends State<QuizDefeatScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('35_QuizDefeat_ResultsScreen', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
