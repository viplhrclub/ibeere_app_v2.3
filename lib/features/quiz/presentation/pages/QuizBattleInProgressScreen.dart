...
class QuizBattleInProgressScreen extends StatefulWidget {
  const QuizBattleInProgressScreen({Key? key}) : super(key: key);
  @override
  State<QuizBattleInProgressScreen> createState() => _QuizBattleInProgressScreenState();
}

class _QuizBattleInProgressScreenState extends State<QuizBattleInProgressScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('51_QuizBattle_InProgress', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
