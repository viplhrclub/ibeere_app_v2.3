...
class QuizBattleCompletionScreen extends StatefulWidget {
  const QuizBattleCompletionScreen({Key? key}) : super(key: key);
  @override
  State<QuizBattleCompletionScreen> createState() => _QuizBattleCompletionScreenState();
}

class _QuizBattleCompletionScreenState extends State<QuizBattleCompletionScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('57_QuizBattleCompletion_Results', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
