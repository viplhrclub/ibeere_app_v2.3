...
class ContestEntryScreen extends StatefulWidget {
  const ContestEntryScreen({Key? key}) : super(key: key);
  @override
  State<ContestEntryScreen> createState() => _ContestEntryScreenState();
}

class _ContestEntryScreenState extends State<ContestEntryScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('38_ContestEntry_ScienceContest', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
