...
class HomeQuizFeedScreen extends StatefulWidget {
  const HomeQuizFeedScreen({Key? key}) : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuestionsCubit, QuestionsState>(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('15_HomeQuizFeed_Navigation', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
