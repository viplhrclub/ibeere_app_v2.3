...
class GuessWordVictoryScreen extends StatefulWidget {
  const GuessWordVictoryScreen({Key? key}) : super(key: key);
  @override
  State<GuessWordVictoryScreen> createState() => _GuessWordVictoryScreenState();
}

class _GuessWordVictoryScreenState extends State<GuessWordVictoryScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('30_GuessWord_VictoryScreen', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
