...
class ProfileStatisticsVariantScreen extends StatefulWidget {
  const ProfileStatisticsVariantScreen({Key? key}) : super(key: key);
  @override
  State<ProfileStatisticsVariantScreen> createState() => _ProfileStatisticsVariantScreenState();
}

class _ProfileStatisticsVariantScreenState extends State<ProfileStatisticsVariantScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('23_ProfileStatistics_Variant2', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
