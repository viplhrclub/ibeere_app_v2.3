...
class OnboardingGetReadyScreen extends StatefulWidget {
  const OnboardingGetReadyScreen({Key? key}) : super(key: key);
  @override
  State<OnboardingGetReadyScreen> createState() => _OnboardingGetReadyScreenState();
}

class _OnboardingGetReadyScreenState extends State<OnboardingGetReadyScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AuthCubit, AuthState>(listener: (ctx, st) { if (st is Authenticated) { WidgetsBinding.instance.addPostFrameCallback((_) { Navigator.pushReplacementNamed(ctx, '/home'); }); } }, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('03_OnboardingScreen_GetReady', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
