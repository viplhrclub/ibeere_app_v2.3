...
class OnboardingKnowledgeScreen extends StatefulWidget {
  const OnboardingKnowledgeScreen({Key? key}) : super(key: key);
  @override
  State<OnboardingKnowledgeScreen> createState() => _OnboardingKnowledgeScreenState();
}

class _OnboardingKnowledgeScreenState extends State<OnboardingKnowledgeScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AuthCubit, AuthState>(listener: (ctx, st) { if (st is Authenticated) { WidgetsBinding.instance.addPostFrameCallback((_) { Navigator.pushReplacementNamed(ctx, '/home'); }); } }, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('02_OnboardingScreen_KnowledgeChallenge', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
