...
class BattleCountdownVariantScreen extends StatefulWidget {
  const BattleCountdownVariantScreen({Key? key}) : super(key: key);
  @override
  State<BattleCountdownVariantScreen> createState() => _BattleCountdownVariantScreenState();
}

class _BattleCountdownVariantScreenState extends State<BattleCountdownVariantScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('37_BattleCountdown_Variant2', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
