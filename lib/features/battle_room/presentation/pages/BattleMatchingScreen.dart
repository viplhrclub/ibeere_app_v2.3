...
class BattleMatchingScreen extends StatefulWidget {
  const BattleMatchingScreen({Key? key}) : super(key: key);
  @override
  State<BattleMatchingScreen> createState() => _BattleMatchingScreenState();
}

class _BattleMatchingScreenState extends State<BattleMatchingScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder(listener: (ctx, st) {}, builder: (context, state) { return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: Center(
          child: Text('48_BattleMatching_OpponentFound', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
