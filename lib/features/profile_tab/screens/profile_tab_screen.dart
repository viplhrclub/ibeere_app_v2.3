...
final class ProfileTabScreenState extends State<ProfileTabScreen>
  ...
  void _onTapMenuItem(String name) {
    ...
    switch (name) {
      case 'theme':
        // Theme selection would be handled through settings
        _showThemeBottomSheet();
        return;
      case 'quizLanguage':
        _showQuizLanguageBottomSheet();
        return;
      case 'language':
      ...
      case 'aboutQuizApp':
        ...
        return;
      case howToPlayLbl:
        globalCtx.pushNamed(Routes.appSettings);
        return;
      case 'shareAppLbl':
        {
          try {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              context.tr('selectTheme') ?? 'Select Theme',
  ...
  void _showQuizLanguageBottomSheet() {
    // Implement quiz language selection
    showModalBottomSheet<void>(
      context: globalCtx,
      builder: (context) => Container(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            Text(
              context.tr('selectQuizLanguage') ?? 'Select Quiz Language',
              style: Theme.of(context).textTheme.titleMedium,
            ),
          ],
        ),
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              context.tr('selectLanguage') ?? 'Select Language',
              style: Theme.of(context).textTheme.titleMedium,
            ),
          ],
        ),
  ...
  void _showLogoutDialog() {
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        title: Text(context.tr('logout') ?? 'Logout'),
        content: Text(context.tr('logoutConfirm') ?? 'Are you sure you want to logout?'),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(context.tr('logout') ?? 'Logout'),
          ),
        ],
      ),
    );
  ...
  void _showDeleteAccountDialog() {
    showDialog<void>(
      context: globalCtx,
      builder: (context) => AlertDialog(
        title: Text(context.tr('deleteAccount') ?? 'Delete Account'),
        content: Text(context.tr('deleteAccountWarning') ?? 'This action cannot be undone. Are you sure?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text(context.tr('cancel') ?? 'Cancel'),
  ...
  Widget _buildProfileHeader() {
    void onTapEditProfile() {
      ...
      globalCtx.pushNamed(Routes.selectProfile);
    }
